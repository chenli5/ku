...
public class ItemsDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory.getLogger(ItemsDAO.class);
	// property constants
	public static final String ITNAME = "itname";
	...
	public Items findById(java.lang.Integer id) {
		...
		try {
			Items instance = (Items) getSession().get("hiber.Items", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Items instance) {
		...
		try {
			List results = getSession().createCriteria("hiber.Items")
	...
	public List findByItname(Object itname) {
		return findByProperty(ITNAME, itname);
	}
